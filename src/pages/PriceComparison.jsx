  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="flex rounded-md justify-center mb-6">
        <button
          className={`px-4 py-2 rounded-s-lg border border-earthy-tan ${
            selectedTab === "division"
              ? "bg-earthy-yellow text-white"
              : "bg-earthy-beige text-earthy-brown"
          }`}
          onClick={() => handleTabChange("division")}>
          Division
        </button>
        <button
          className={`px-4 py-2 border-t border-b border-earthy-tan ${
        ...
        <button
          className={`px-4 py-2 rounded-e-lg border border-earthy-tan ${
            selectedTab === "subdistrict"
              ? "bg-earthy-yellow text-white"
              : "bg-earthy-beige text-earthy-brown"
          }`}
      ...
      <div>
        {selectedTab === "division" && (
          <DivitionsByCategory categoryId={categoryId} />
        )}
        {selectedTab === "district" && (
          <DistrictsByCategory categoryId={categoryId} />
        )}
        {selectedTab === "subdistrict" && (
          <SubDistrictsByCategory categoryId={categoryId} />
        )}
      </div>
    </div>
  );
